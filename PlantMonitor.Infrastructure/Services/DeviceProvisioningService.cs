...
public class DeviceProvisioningService(IApplicationDbContext context, ILogger<DeviceProvisioningService> logger)
    : IDeviceProvisioningService
{
    public async Task<DeviceProvisioningResult> ProvisionNewDeviceAsync(string deviceId)
    {
        try
        {
            var existingToken = await context.ApiTokens
        ...
        {
            logger.LogError(ex, "Error provisioning device {DeviceId}", deviceId);
            return new DeviceProvisioningResult
            {
                DeviceId = deviceId,
                Token = string.Empty,
